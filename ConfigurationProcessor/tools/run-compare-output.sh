...
then
    ...
    case ${RUN_VERSION} in
        "2.0")
            ...
            OPTIONS="${OPTIONS} -s .test/ardana-input-model/2.0/"
            ;;
        "1.0")
...
then
    /usr/zuul-env/bin/zuul-cloner \
        ...
        --cache-dir /opt/git ${GIT_MIRROR:-https://gerrit.suse.provo.cloud/p} \
        --project-branch ardana/ardana-input-model=${BRANCH} \
        ardana/ardana-input-model
else
    git clone --depth 1 --single-branch -b ${BRANCH} \
        https://gerrit.suse.provo.cloud/p/ardana/ardana-input-model ardana-input-model
fi

[[ "${PROJECT}" == "ardana/ardana-input-model" ]] && PUSHD=${PWD}/ardana-input-model
popd
