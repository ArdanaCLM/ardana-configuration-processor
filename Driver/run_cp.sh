...
DEFINITION=<path-to-ardana-input-model>/2.0/examples/entry-scale-kvm-vsa/cloudConfig.yml


# This is where the service inputs are located
SERVICE_DIR=<path-to-ardana-input-model>/2.0/
...
python ./ardana-cp -s ${SERVICE_DIR} -r ${SCHEMA_DIR} -c ${DEFINITION} ${ENCRYPTION} ${REFRESH_PASSWORDS} ${WRITE_LOCAL} ${QUIET} ${ENCRYPTION_KEY} ${REKEY} ${REMOVE_DELETED_SERVERS} ${FREE_UNUSED_ADDRESSES}
